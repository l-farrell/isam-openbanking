          ...
          <stsuuser:STSUniversalUser xmlns:stsuuser="urn:ibm:names:ITFIM:1.0:stsuuser">
            ...
            <stsuuser:ContextAttributes>
              <stsuuser:Attribute name="claim_json" type="">
                <stsuuser:Value>{"aud":"isamob","iss":"https://isam.local", "sub": "@CLIENT_ID@","exp":1608178320}</stsuuser:Value>
              </stsuuser:Attribute>
...
CLIENT_XML_DOC=$(echo $XML_DOC | sed  "s/@CLIENT_ID@/$CLIENT_ID/g")

OUTPUT=$(curl -s -S -k --user easuser:passw0rd -H "x-forwarded-by" -H "Content-Type: text/xml" -d "$CLIENT_XML_DOC"  https://isam.local:20443/TrustServerWS/SecurityTokenServiceWST13)
